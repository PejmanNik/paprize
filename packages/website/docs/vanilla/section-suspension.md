...
By providing a list of promises in the `suspense` option of [SectionOptions](/core/api#sectionoptions), you can control the section’s suspension. When you [schedule a pagination operation](api#schedulepagination) and a section has pending promises in `suspense`, the pagination engine will skip that section until all promises for that section are resolved or rejected.
